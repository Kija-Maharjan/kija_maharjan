  ...
  return (
    <AdminLayout title="Certificates">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="text-xs text-text-dim tracking-wide">{certs.length} certificates</div>
      ...
      {showForm && (
        <div className="bg-dark-2 p-6 border border-gold/10 mb-8">
          <div className="text-[9px] tracking-[3px] uppercase text-gold mb-5">Add Certificate</div>
          <form onSubmit={addCert}>
            <div className="form-row mb-4">
              <div>
                <label className="text-[9px] tracking-[2px] uppercase text-text-dim block mb-2">Certificate Name *</label>
              ...
              <div>
                <label className="text-[9px] tracking-[2px] uppercase text-text-dim block mb-2">Issuer *</label>
            ...
            <div className="form-row mb-4">
              <div>
                <label className="text-[9px] tracking-[2px] uppercase text-text-dim block mb-2">Date</label>
                <input className="form-input" type="date" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} />
              </div>
              <div>
                <label className="text-[9px] tracking-[2px] uppercase text-text-dim block mb-2">Status</label>
            ...
            <div className="mb-4">
              <label className="text-[9px] tracking-[2px] uppercase text-text-dim block mb-2">Certificate URL (optional)</label>
      ...
      <div className="flex flex-col gap-0.5">
        {certs.map(cert => (
          <div key={cert.id} className="bg-dark-2 p-5 md:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border border-gold/5 hover:border-gold/20 transition-colors">
            <div>
              <div className="font-serif text-lg text-cream">{cert.name}</div>
              <div className="text-[10px] tracking-[2px] uppercase text-text-dim mt-1">{cert.issuer} {cert.date && `· ${cert.date}`}</div>
            </div>
